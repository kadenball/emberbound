...
try {
 ...
 const output=process.argv[2]??'docs/HEAVY-SPAM-AUDIT-0.29.json';
 ...
 if(!profiles.length)for(const hero of HEROES){
  ...
  for(let stage=0;stage<STAGES.length;stage++){
   ...
   for(let attempt=0;attempt<3&&!won;attempt++){
    ...
    for(let frame=0;frame<18000&&g.state==='playing';frame++){g.update(1/60,playInput(g,frame));g.events=[];}
 ...
 const rows=[];
 for(const profile of profiles)for(const seed of [7,140,991])for(const policy of ['fast-heavy','stationary-heavy']){
  ...
  for(;frame<18000&&g.state==='playing';frame++){
   ...
   if(target&&policy!=='mixed'){
    ...
    input.attack=false;input.heavy=close&&frame%2===0;if(policy==='stationary-heavy'&&close){input.x=0;input.y=0;}
 ...
 const summaries=['fast-heavy','stationary-heavy'].map(policy=>{const group=rows.filter(r=>r.policy===policy);return {policy,runs:group.length,wins:group.filter(r=>r.result==='won').length,losses:group.filter(r=>r.result==='lost').length,timeouts:group.filter(r=>r.result==='playing').length,bossWins:group.filter(r=>r.stage%2&&r.result==='won').length};});
 await writeFile(output,JSON.stringify({date:new Date().toISOString(),method:'Two rapid Heavy policies × three seeds × three heroes × twelve stages; one taps every other frame while pursuing and aligning, the other stops moving whenever a target is in reach; identical profiles earned by the mixed Scrapper campaign probe. Reused baseline profiles when comparing changes. All policies can work mandatory scenery after enemies die. No health edits, teleportation or wave skipping. Scripted steering is not human fun or difficulty evidence.',profiles,summaries,rows},null,2)+'\n');console.log(JSON.stringify(summaries,null,2));
